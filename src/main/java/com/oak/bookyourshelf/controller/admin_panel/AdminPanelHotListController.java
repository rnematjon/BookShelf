...
import com.oak.bookyourshelf.entity.*;
...
public class AdminPanelHotListController {
    ...
    public String tab(@RequestParam("page") Optional<Integer> page,
        ...
        GlobalConstants.getPageNumbers(page, size, filterHotLists(adminPanelHotListService.sortHotlists(currentSort), currentFilter),
                model, "hotListPage");
    ...
    public List<String> findAllSubcategories(@RequestParam String category) {
        return GlobalConstants.getAllSubcategories(adminPanelCategoryService.getByName(category));
    }
            ...
            if (subctgry.equals("")) {
                for (HotList i : sameType) {
                    if (i.getCategories().size() != 0) {
                        ...
                        return ResponseEntity.badRequest().body("There is a hot list in " + GlobalConstants.productTypeNames.get(i.getProductType().toString()) + " . Please change your selection");
                    }
                }
